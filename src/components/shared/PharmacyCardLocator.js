...
import api from '../../services/api';  //[OKS] Adjust the import path as necessary
...
// [OKS *0-0*] Fetch pharmacies from backend API
  const fetchPharmacies = async () => {
    try {
      setLoading(true);
      setError(null);
      
      const response = await  api.pharmacies.findNearbyPharmacist();
  ...
  const useDefaultLocation = () => {
    const minyaLocation = { lat: 28.1099, lng: 30.7503 };
    setUserLocation(minyaLocation);
    setMapCenter(minyaLocation);
    fetchPharmacies();
  };

  // [OKS] Initialize Mapbox Map
  useEffect(() => {
 ...
 //[OKS] user location marker
  ...
  // [OKS] Update map markers when pharmacies change
  useEffect(() => {
    if (!map.current || !mapLoaded || pharmacies.length === 0) return;

    //[OKS]  Clear existing markers
    markers.current.forEach(marker => marker.remove());
    markers.current = [];

    // [OKS] Add pharmacy markers
    pharmacies.forEach(pharmacy => {
      ...
      // [OKS] Highlight selected pharmacy
    ...
    //  [OKS] Center map on selected pharmacy
    if (selectedPharmacy) {
      map.current.flyTo({

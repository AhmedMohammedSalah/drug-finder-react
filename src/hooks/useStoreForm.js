...
const useStoreForm = () => {
        ...
        if (pharmacist.has_store && pharmacist.medical_stores_data) {
          ...
          if (store.store_logo) {
            const fullLogoUrl = store.store_logo.startsWith("http")
              ? store.store_logo
              : `https://ahmedmsalah.pythonanywhere.com${store.store_logo}`;
  ...
  return {
    ...
    mapLoading,
    handleLatChange: (e) =>
      setLatLng((prev) => ({ ...prev, lat: parseFloat(e.target.value) })),
    handleLngChange: (e) =>
      setLatLng((prev) => ({ ...prev, lng: parseFloat(e.target.value) })),
    handleSubmit,
    errors,
    isEditMode,
    setIsEditMode,

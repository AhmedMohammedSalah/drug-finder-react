...
const DeliveriesAdminPage = () => {
  ...
  useEffect(() => {
    const fetchDeliveries = async () => {
      try {
        ...
        const res = await axios.get('http://localhost:8000/users/deliveries/', {
  ...
  const updateStatistics = (data) => {
    const pending = data.filter(d => d.license_status === 'pending').length;
    const approved = data.filter(d => d.license_status === 'approved').length;
    const rejected = data.filter(d => d.license_status === 'rejected').length;
  ...
  const handleFilterChange = (filters) => {
    ...
    if (filters.status !== 'all') {
      filtered = filtered.filter(d => d.license_status === filters.status);
    }
  ...
  const handleUpdateDelivery = async (id, newStatus, rejectMessage = '') => {
    try {
      const response = await axios.patch(
        `http://localhost:8000/users/deliveries/${id}/`,
        {
      ...
      setDeliveries(prev => prev.map(d => 
        d.id === id ? { ...d, license_status: newStatus } : d
      ));
  ...
  return (
    <div className="relative min-h-screen">
      <AdminLoader loading={loading} error={error} loadingMessage="Loading delivery requests..." />
      <div className={`container mx-auto px-4 pb-24 ${loading || error ? 'opacity-50 pointer-events-none select-none' : 'opacity-100'}`}>
        <h1 className="text-3xl font-bold text-gray-800 mb-6 pt-6">Delivery Requests</h1>
            ...
            <div className="overflow-y-auto pb-4" style={{ maxHeight: 'calc(100vh - 220px)' }}>
              {filteredDeliveries.length === 0 && !loading ? (
                <div className="bg-white p-10 rounded-lg shadow text-center flex flex-col items-center justify-center gap-4 text-gray-500">
                  ...
                  <p className="text-lg font-medium text-gray-600">No deliveries found</p>
                  <p className="text-sm text-gray-400">Try adjusting the filters to see different results.</p>
                </div>
              ) : (
                <div className="space-y-4 pr-2">
                  {getCurrentItems().map(delivery => (
                    <PharmacistRequestCard
                      key={delivery.id}
                      pharmacist={delivery} // reuse card for delivery
      ...
      {selectedDelivery && (
        <PharmacistModal
          pharmacist={selectedDelivery} // reuse modal for delivery
...
export default DeliveriesAdminPage; 
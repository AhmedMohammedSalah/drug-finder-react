...
function OrdersAdminPage() {
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`https://ahmedmsalah.pythonanywhere.com/orders/${id}/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="relative min-h-screen">
      <AdminLoader
        loading={loading}
        error={error}
        loadingMessage="Loading orders data..."
      />
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    <option value="">Select Status</option>
                    {editOrder?.order_status !== "pending" && (
                      <option value="pending">Pending</option>
                    )}
                    {editOrder?.order_status !== "paid" && (
                      <option value="paid">Paid</option>
                    )}
                    {editOrder?.order_status !== "on_process" && (
                      <option value="on_process">On Process</option>
                    )}
                    {editOrder?.order_status !== "shipping" && (
                      <option value="shipping">Shipping</option>
                    )}
                    {editOrder?.order_status !== "delivered" && (
                      <option value="delivered">Delivered</option>
                    )}
                    {editOrder?.order_status !== "canceled" && (
                      <option value="canceled">Canceled</option>
                    )}
                  </select>
                </div>

...
function MedicineAdminPage() {
  ...
  useEffect(() => {
    setLoading(true);
    const url = `http://localhost:8000/inventory/medicines/?page=${page}&page_size=${pageSize}`;
    fetch(url, {
      headers: {
  ...
  useEffect(() => {
    async function fetchStores() {
      const uniqueStoreIds = Array.from(new Set((medicines || []).map(m => m.store).filter(Boolean)));
      const newStoreData = { ...storeData };
      await Promise.all(uniqueStoreIds.map(async storeId => {
        if (!newStoreData[storeId]) {
          try {
            const res = await fetch(`http://localhost:8000/medical_stores/${storeId}/`, {
  ...
  return (
    <div className="relative min-h-screen">
          ...
          <div className="flex flex-col items-center gap-4 p-8 bg-white/60 rounded-xl shadow-lg">
            {loading && (
              <>
                ...
                <span className="text-lg font-semibold text-blue-700">Loading medicine data...</span>
              </>
            )}
            {error && (
              <>
      ...
      <div className={`container mx-auto px-4 pb-24 ${loading || error ? 'opacity-50 pointer-events-none select-none' : 'opacity-100'}`}>
        <div className={`flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-white rounded-lg p-2 sm:p-4 md:p-6 ${loading ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
          <div className="w-full max-w-5xl flex flex-col items-center mb-4 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-center mb-4 sm:mb-6 text-blue-700 drop-shadow">Medicines</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 w-full max-w-5xl">
            {filteredMedicines.map(med => (
              <div key={med.id} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-3 border border-blue-100 hover:shadow-xl transition-shadow duration-200 relative">
                <div className="font-bold text-xl text-blue-800 flex items-center gap-2">{med.generic_name} <span className="text-gray-500 font-normal">({med.brand_name})</span></div>
                <div className="text-gray-600 text-sm">Chemical: <span className="font-semibold text-blue-600">{med.chemical_name}</span></div>
                ...
                <div className="text-xs text-blue-500 font-semibold flex flex-col">
                  {med.store && storeData[med.store] ? (
                    <span>Store: {storeData[med.store].store_name} <span className="text-gray-500">({storeData[med.store].store_type})</span></span>
                  ) : (
                    med.store ? <span>Store ID: {med.store}</span> : <span>No Store</span>
                  )}
                </div>
                <div className="flex gap-2 mt-2 justify-end">
                  <button className="bg-green-500 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition-all" onClick={() => handleEdit(med)}>Edit</button>
                  <button className="bg-red-500 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition-all" onClick={() => setDeleteMedicineId(med.id)}>Delete</button>
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="fixed inset-0 flex items-center justify-center z-50">
                <form onSubmit={handleAddSubmit} className="bg-white rounded-2xl p-4 sm:p-8 shadow-2xl flex flex-col gap-6 min-w-[90vw] sm:min-w-[350px] max-w-md max-h-[90vh] overflow-y-auto border-2 border-blue-200 relative animate-fadeIn">
                  <div className="flex flex-col items-center justify-center rounded-t-3xl bg-gradient-to-r from-blue-500 to-blue-400 p-6">
                    ...
                    <h2 className="text-2xl font-extrabold text-blue-800 mb-0 tracking-wide">Add New Medicine</h2>
                  </div>
                  <div className="flex flex-col gap-4 p-8 pt-6">
                    <input name="generic_name" value={addForm.generic_name || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Generic Name" required />
                    <input name="brand_name" value={addForm.brand_name || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Brand Name" required />
                    <input name="chemical_name" value={addForm.chemical_name || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Chemical Name" required />
                    <input name="description" value={addForm.description || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Description" />
                    <input name="atc_code" value={addForm.atc_code || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="ATC Code" />
                    <input name="cas_number" value={addForm.cas_number || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="CAS Number" />
                    <input name="stock" value={addForm.stock || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Stock" type="number" required />
                    <input name="price" value={addForm.price || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Price" type="number" step="0.01" required />
                    {/* Store select for Add modal */}
                    <select name="store" value={addForm.store || ''} onChange={handleAddChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" required>
                      <option value="">Select Store</option>
                      {allStores.map(store => (
                        <option key={store.id} value={store.id}>{store.store_name} ({store.store_type})</option>
                      ))}
                    </select>
              ...
              <div className="fixed inset-0 flex items-center justify-center z-50">
                <form onSubmit={handleEditSubmit} className="bg-white rounded-2xl p-4 sm:p-8 shadow-2xl flex flex-col gap-6 min-w-[90vw] sm:min-w-[350px] max-w-md max-h-[90vh] overflow-y-auto border-2 border-blue-200 relative animate-fadeIn">
                  <div className="flex flex-col items-center justify-center rounded-t-3xl bg-gradient-to-r from-blue-500 to-blue-400 p-6">
                    ...
                    <h2 className="text-2xl font-extrabold text-blue-800 mb-0 tracking-wide">Edit Medicine</h2>
                  </div>
                  <div className="flex flex-col gap-4 p-8 pt-6">
                    <input name="generic_name" value={editForm.generic_name || ''} onChange={handleEditChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Generic Name" required />
                    <input name="brand_name" value={editForm.brand_name || ''} onChange={handleEditChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Brand Name" required />
                    <input name="chemical_name" value={editForm.chemical_name || ''} onChange={handleEditChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Chemical Name" required />
                    <input name="description" value={editForm.description || ''} onChange={handleEditChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Description" />
                    <input name="atc_code" value={editForm.atc_code || ''} onChange={handleEditChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="ATC Code" />
                    <input name="cas_number" value={editForm.cas_number || ''} onChange={handleEditChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="CAS Number" />
                    <input name="stock" value={editForm.stock || ''} onChange={handleEditChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Stock" type="number" required />
                    <input name="price" value={editForm.price || ''} onChange={handleEditChange} className="border border-blue-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400" placeholder="Price" type="number" step="0.01" required />
                    ...
                    <div className="flex gap-4 mt-2 justify-end">
                      ...
                      <button type="button" className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-8 py-3 rounded-xl font-semibold shadow transition-all focus:outline-none" onClick={() => setEditMedicine(null)}>Cancel</button>
                    </div>
                  </div>
                  <button type="button" className="absolute top-4 right-4 text-gray-400 hover:text-red-500 text-3xl font-bold focus:outline-none transition-all duration-150" onClick={() => setEditMedicine(null)} aria-label="Close">&times;</button>
                </form>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="fixed inset-0 flex items-center justify-center z-50">
                <div className="bg-white rounded-2xl p-8 shadow-2xl flex flex-col gap-6 min-w-[300px] max-w-[95vw] max-h-[90vh] overflow-y-auto border-2 border-red-200 relative animate-fadeIn">
                  ...
                  <div className="flex flex-col gap-4 p-8 pt-6">
                    <p className="text-gray-700">Are you sure you want to delete this medicine?</p>
                    <div className="flex gap-4 mt-2 justify-end">
                      <button onClick={async () => { await handleDelete(deleteMedicineId); setDeleteMedicineId(null); }} className="bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded-xl font-bold shadow transition-all focus:outline-none focus:ring-2 focus:ring-red-400">Delete</button>
                      <button onClick={() => setDeleteMedicineId(null)} className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-8 py-3 rounded-xl font-semibold shadow transition-all focus:outline-none">Cancel</button>
                    </div>
                  </div>
                  <button type="button" className="absolute top-4 right-4 text-gray-400 hover:text-red-500 text-3xl font-bold focus:outline-none transition-all duration-150" onClick={() => setDeleteMedicineId(null)} aria-label="Close">&times;</button>
                </div>
              </div>
            </>
          )}

          ...
          <div className="flex flex-col items-center gap-4 p-8 bg-white/60 rounded-xl shadow-lg">
            {loading && (
              <>
                ...
                <span className="text-lg font-semibold text-blue-700">Loading orders data...</span>
              </>
            )}
            {error && (
              <>
      ...
      <div className={`container mx-auto px-4 pb-24 ${loading || error ? 'opacity-50 pointer-events-none select-none' : 'opacity-100'}`}>
        <div className={`flex flex-col items-center justify-center min-h-screen bg-gray-100 rounded-lg p-2 sm:p-4 md:p-6 ${loading ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
          <div className="w-full max-w-5xl flex flex-col items-center mb-4 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-center mb-4 sm:mb-6 text-blue-700 drop-shadow">Orders Page</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 w-full max-w-5xl">
            {filteredOrders.length === 0 ? (
              <div className="col-span-full text-center text-gray-500 text-lg py-12">No orders found.</div>
            ) : (
              filteredOrders.map(order => {
                // console.log('Rendering order store:', order.store);
                const isOpen = detailsOrder && detailsOrder.id === order.id;
                return (
                  <div key={order.id} className="bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-2 border border-blue-100 hover:shadow-xl transition-shadow duration-200 w-full col-span-full">
...
function ClientDetails({ client, fetchClientById }) {
  ...
  React.useEffect(() => {
    ...
    async function fetchUserName() {
      if (clientObj && clientObj.user && typeof clientObj.user === 'number') {
        try {
          const res = await fetch(`http://localhost:8000/users/users/${clientObj.user}/`, {
  ...
  if (loading) return <span className="text-gray-400 text-xs">Loading client...</span>;
  if (!clientObj) return <span className="text-gray-400 text-xs">Unknown client</span>;
  ...
  return (
    <div className="flex items-center gap-3 mt-1">
      <img src={imageUrl} alt="Profile" className="w-10 h-10 rounded-full object-cover border border-gray-300" />
      <div className="flex flex-col text-xs">
        <span className="font-semibold text-gray-800">User: {userName || '-'}</span>
        {/* <span className="text-gray-600">{clientObj.email || '-'}</span> */}
        {clientObj.info_disease && (
          <span className="text-gray-500 italic">Disease: {clientObj.info_disease}</span>
        )}
      </div>
    </div>
  );

...
function UsersAdminPage() {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      ...
      if (addForm.role === 'pharmacist') {
      ...
      } else if (addForm.role === 'client') {
        // Client: send JSON with profile image if present
        const formData = new FormData();
        formData.append('name', addForm.name);
        formData.append('email', addForm.email);
        formData.append('password', addForm.password);
        ...
        // Admin or other roles: send JSON
        const res = await fetch('http://localhost:8000/users/users/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  // [SARA]: Enhanced filter users by search query (name, email, role) and role
  const filteredUsers = users.filter(user => {
    ...
    return matchesRole && (searchLower === '' || matchesSearch);
  });

  // [SARA]: Responsive filter bar state
  const [showFilters, setShowFilters] = useState(false);
  const isMobile = typeof window !== 'undefined' && window.innerWidth < 640;

  // [SARA]: Helper to clear all filters
  const clearFilters = () => {
    setSearch('');
    setRoleFilter('');
  };
  ...
  return (
    <div className="relative min-h-screen">
          ...
          <div className="flex flex-col items-center gap-4 p-8 bg-white/60 rounded-xl shadow-lg">
            {loading && (
              <>
                ...
                <span className="text-lg font-semibold text-blue-700">Loading users data...</span>
              </>
            )}
            {error && (
              <>
      ...
      <div className={`container mx-auto px-4 pb-24 ${loading || error ? 'opacity-50 pointer-events-none select-none' : 'opacity-100'}`}>
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-white rounded-lg p-2 sm:p-4 md:p-6">
          <div className="w-full max-w-5xl flex flex-col items-center mb-4 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-center mb-4 sm:mb-6 text-blue-700 drop-shadow">Users</h1>
            {/* Responsive filter bar */}
            <div className="w-full">
              {/* Mobile: sticky, collapsible filter bar */}
              <div className="sm:hidden sticky top-0 z-30 bg-gradient-to-br from-blue-50 to-white rounded-xl shadow border border-blue-200 mb-2">
                <div className="flex items-center justify-between px-4 py-2">
                  <div className="flex gap-2 items-center">
                    <button
                      className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-bold shadow transition-all focus:outline-none"
                      onClick={() => setShowFilters(v => !v)}
                      aria-label="Toggle filters"
                    >
                      <svg className="w-5 h-5 inline-block mr-1" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" /></svg>
                      Filters
                    </button>
                    {/* Filter chips */}
                    {search && (
                      <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded-full text-xs ml-1">Search: {search}</span>
                    )}
                    {roleFilter && (
                      <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded-full text-xs ml-1">Role: {roleFilter}</span>
                    )}
                  </div>
                  {(search || roleFilter) && (
                    <button
                      className="text-blue-600 hover:text-red-500 text-xs font-semibold ml-2 underline"
                ...
                {showFilters && (
                  <div className="flex flex-col gap-3 px-4 pb-4 animate-fadeIn">
                    <div className="flex items-center gap-3 bg-white bg-opacity-90 rounded-xl shadow px-2 py-2 border border-blue-200 w-full">
                    ...
                    <div className="flex items-center gap-2 bg-white bg-opacity-90 rounded-xl shadow px-2 py-2 border border-blue-200 min-w-[140px] w-full">
              ...
              <div className="hidden sm:flex w-full flex-row items-center justify-between gap-4 mb-4">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-2 bg-white bg-opacity-90 rounded-xl shadow px-2 sm:px-4 py-2 sm:py-3 border border-blue-200 min-w-[140px] sm:min-w-[180px] mt-2 sm:mt-0">
                  <svg className="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" /></svg>
                  <select
                    value={roleFilter}
                    onChange={e => setRoleFilter(e.target.value)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 w-full max-w-5xl">
            {filteredUsers.length === 0 ? (
              <div className="col-span-full text-center text-gray-500 text-lg py-12">No users found matching your filter.</div>
            ) : (
